...
export class DecreaseIndentToolBar extends SlateTool implements TSlateTool {
  static namespace = 'DecreaseIndentToolbar';
  ...
  render(props: TToolProps) {
    ...
    const onClick = useCallback((e: React.MouseEvent<HTMLSpanElement, MouseEvent>) => {
      ...
      if (props.status !== 'disabled') {
        this.container.cast('editor:' + TextIndentFunction.namespace, { textIndent: Number(value) - 2 <= 0 ? 0 :  Number(value) - 2 });
      }
    }, [props.status, value]);


    return <span onMouseDown={onClick} className={classnames(props.status, props.className)}>{DecreaseIndentToolBar.icon || 'D'}</span>;
  }
